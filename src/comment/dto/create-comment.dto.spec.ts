...
describe(`Create Comment DTO`, () => {
  ...
  describe(`parent Id`, () => {
    it(`should fail if parentId is not Integer`, async () => {
      const mockObject = {
        parentId: `123`,
      };
      const mockedInstance = plainToInstance(CreateCommentDto, mockObject);
      ...
      const errors = await validate(mockedInstance);
      const parentIdErrors = errors.filter(
        (err) => err.property === `parentId`,
      )[0];

      expect(errors.length).not.toBe(0);
      expect(parentIdErrors).toBeDefined();
      expect(parentIdErrors.constraints).toEqual(
        expect.objectContaining({
          isInt: expect.any(String),
        }),
      );
    ...
    it(`should pass if the parentId is correct`, async () => {
      // create a parentId with correct format
      const mockObject = { parentId: 1 };
      const mockedInstance = plainToInstance(CreateCommentDto, mockObject);
      ...
      expect(parentIdErrors).not.toBeDefined();
      expect.hasAssertions();
    });

    it(`should pass if the parentId is not provided`, async () => {
      // create a parentId with correct format
      const mockObject = {};
      const mockedInstance = plainToInstance(CreateCommentDto, mockObject);
